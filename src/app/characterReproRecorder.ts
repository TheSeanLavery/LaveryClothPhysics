export type CharacterReproEvent =
  | CharacterReproPointerEvent
  | CharacterReproActionEvent
  | CharacterReproNoteEvent;

export interface CharacterReproPointerEvent {
...
export interface CharacterReproActionEvent {
...
export interface CharacterReproNoteEvent {
  readonly type: 'note';
  readonly t: number;
  readonly message: string;
}

export interface CharacterReproRecording {
  readonly schemaVersion: 1;
  readonly kind: 'character-sleeve-repro';
  ...
  readonly events: readonly CharacterReproEvent[];
}

export interface CharacterReproSaveResult {
  readonly ok: boolean;
...
export interface CharacterReproRecorder {
  readonly isRecording: () => boolean;
  readonly start: () => Promise<void>;
  readonly stopAndSave: () => Promise<CharacterReproSaveResult>;
  readonly recordPointer: (phase: CharacterReproPointerEvent['phase'], event: PointerEvent) => void;
  readonly recordAction: (name: string, details?: Record<string, unknown>) => void;
  readonly recordNote: (message: string) => void;
}

export interface CharacterReproRecorderOptions {
...
export function createCharacterReproRecorder(
  ...
  let events: CharacterReproEvent[] = [];

  const relativeTime = (): number => Math.round((performance.now() - startTime) * 1000) / 1000;

  const buildRecording = async (): Promise<CharacterReproRecording> => ({
    schemaVersion: 1,
  ...
  const downloadRecording = (recordingData: CharacterReproRecording): void => {
    const blob = new Blob([`${JSON.stringify(recordingData, null, 2)}\n`], {
      type: 'application/json',
    });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `character-sleeve-repro-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
  ...
  const saveRecording = async (recordingData: CharacterReproRecording): Promise<CharacterReproSaveResult> => {
    try {
      const response = await fetch('/__recordings/character-repro', {
        method: 'POST',
