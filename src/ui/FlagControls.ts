...
import type { FlagSimulation } from '../sim/FlagSimulation';

export function createFlagControls(sim: FlagSimulation): GUI {
  const gui = new GUI({ title: 'Flag Controls', width: 320 });
  ...
  physicsFolder.add(settings, 'solverIterations', 1, 32, 1).name('Solver iterations').onChange(sync);
  physicsFolder.add(settings, 'stiffness', 0, 5, 0.01).name('Stiffness').onChange(sync);
